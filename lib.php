<?php
	...
	if(!isset($_COOKIE[storesettings]) || !$_COOKIE[storesettings]){
		//get store fields settings (from webservice)
		$GLOBALS[storesettings]=get_storesettings(get_storefields());//array of all settings
		
		//Store the Store code in a cookie for 5 Minutes
		setcookie("storesettings",get_storefields(),time()+300);
	}
	else{
		//get store fields settings (from cookie)
		$GLOBALS[storesettings]=get_storesettings($_COOKIE[storesettings]);
	}
	...
	function consumeWebservice2($webservice,$parameters=0){
		...
		$xml = simplexml_load_file($urlquery);
		return $xml;
	}//end consumeWebservice2()
	
	/************************************************* get_storefields *****************************************************/
	//get list of client's store settings [price, orign, ... etc] [return array of all specs]
	function get_storefields(){
		//get storefields 
	...
	/************************************************* get_storesettings *****************************************************/
	//get list of client's store settings [price, orign, ... etc] [return array of all specs]
	function get_storesettings($storecode){
